...
onMounted(async () => {
    ...
    const [upcomingRes, pastRes] = await Promise.all([
        ...
        // Past events are typically archived once they're over, so the past
        // list intentionally does NOT filter on is_archived (the DB policy must
        // also allow anon to read archived rows). Drafts stay hidden.
        supabase
            .from('events')
            .select('id, title, artwork_url, location, event_date')
            .eq('is_draft', false)

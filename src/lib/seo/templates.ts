import { absoluteUrl, extractCity, formatEventDateLong, formatEventDateShort, truncate } from "./derive";
import { SITE_NAME, SITE_ORIGIN, type JsonLd, type MetaTag, type SeoEvent, type SeoMeta } from "./types";

const HOME_TITLE = `${SITE_NAME} — Record label based in lucerne`;
const HOME_DESCRIPTION = `${SITE_NAME} is a house music record label presenting underground events in Lucerne and beyond. Upcoming shows, tickets, radios, mixes.`;
...
export function buildHomeMeta(): SeoMeta {
    ...
    const ogDescription = truncate("Electronic events in Lucerne & Switzerland.", MAX_OG_DESCRIPTION);
    return {
        title,
        description,
...
const SOCIAL_LINKS = ["https://www.instagram.com/eclipse_boundaries/", "https://www.youtube.com/@eclipse_boundaries", "https://tiktok.com/@eclipse_boundaries"];
